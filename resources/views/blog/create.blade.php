...
@endif
<form method="POST" action="{{ route('blogs.store') }}" id="create_post">
    @csrf
    Title: <input type="text" name="title" id="title" required>
    Body: <textarea name="body" id="body" required></textarea>
    <button type="button" onclick="save_post()">Save</button>
</form>
       <script>
           function save_post() {
               if($("#title").val()==""){
